 ...
 * Dec 2, 2011
 */
public class AsyncCommandRunnerIgnoresStreamCaptureTest extends TestCase {
	...
	public void testRunCommandSimpleSuccessWithNoCallback() throws InterruptedException {
		AsyncCommandRunnerIgnoresStreamCapture runner = new AsyncCommandRunnerIgnoresStreamCapture(null);
		...
		// Wait 5s to see if the output is gathered. Else tweak this number. Junit exits unfortunately before other 
		// thread finishes. 
		Thread.sleep(5000);
		
		Assert.assertNull(response.getSuccessResponseOutputStream());
	...
	public void testRunCommandSimpleSuccessWithCallback() throws InterruptedException {
		...
		AsyncCommandRunnerIgnoresStreamCapture runner = new AsyncCommandRunnerIgnoresStreamCapture(CommandUtilForTest.anonAsyncHandler(msg));
		CommandResponse response = new CommandResponse();
		CommandRequest request = CommandUtilForTest.simpleCommand();
